...
- 暂停期间不写入正式样本
- 入口和出口RTK端点仅响应操作员手动命令，不在开始或停止时隐式抓取；待执行任务的入口命令先写入任务目录`entry_rtk_snapshot.db`，开始时原子导入正式测量库并清理暂存主库及WAL边文件
- 只有最近 2 s 内收到的有效 Fix 才确认入口或出口坐标；失败的重新记录不会覆盖此前已确认端点
- 停止时先固定测量边界并立即收尾，不等待RTK；未手动记录的端点保持`not_requested`
- 停止后可短暂重开既有`measurements.db`补录出口RTK；该事务只更新端点、事件和出口状态，不修改正式样本、计数、停止时间或完成标志
- 原始RTK保存到 `rtk_samples`；旧三张融合定位表保留schema兼容，新任务不写伪数据
- 正式文件先写入 `measurements.db.tmp`，正常或异常收尾后重命名为 `measurements.db`

记录器保留 `lidar_to_top_m` 作为算法原始输出，并将正式字段 `clearance_height_m` 写为 `lidar_to_top_m + lidar_mount_height_m`。算法 Topic 不改写。记录格式 schema version 5 在 `clearance_source_frames` 使用 `candidate_region_count`、`selected_grid_area_m2`、`selected_residual_median_m` 和 `selected_residual_p95_m` 保存明确的算法诊断语义，并在 `recording_metadata` 中保存实际 `travel_direction` 和 `lane_side`；兼容字段 `lane` 继续保留。schema version 6 至 12 保留历史演进语义，version 13 新增按 IMU 实际接收频率写入的 `imu_samples` 原始表，version 14 新增任务开始时冻结的 `detection_radius_m` 和 `min_support_points`，version 15 新增 `lane_number_from_right`，其 1 至 4 表示从最右侧向左计数的车道。后端兼容 version 1 至 15。
