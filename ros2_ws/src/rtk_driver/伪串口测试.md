...
- 浏览器顶部RTK定位栏能够显示RTK连接状态、原始经纬高、卫星数和HDOP/PDOP。
...
| 顶部RTK定位栏 | 有效原始 WGS84 经纬高、卫星数和HDOP/PDOP |
| 顶部RTK定位栏卫星数 | `14` |
| 顶部RTK定位栏 HDOP / PDOP | `1.10 / 2.60` |
| 顶部RTK定位栏高度 | 约 `14.21 m` |

`gps_state` 和 RMC 字段仍通过 WebSocket 传输，但顶部RTK定位栏不单独显示这两项。坐标由NMEA度分格式转换为WGS84十进制度后显示，预期约为：
