...
├── clearance_preview.launch.py   # 原始雷达本体系净空算法
├── cloud_preview.launch.py       # 原始雷达X-Y-Z点云预览节点
...
默认只加载唯一正式配置 `clearance_engine.yaml`，并启动 `clearance_engine_node`。
净空节点直接订阅 `/capture/lidar/points_raw`，执行原始雷达本体系圆柱 ROI 和最低可信点簇
算法；不订阅 IMU、里程计，也不进行旋转、平移或运动补偿。
...
输入 `/capture/lidar/points_raw`，保持原始 X-Y-Z 坐标并输出
`/capture/visualization/cloud_preview`。节点按同帧原始索引合并净空诊断，
...
`config/dev_parameter_bindings.yaml` 定义 development 调试参数，包括参数键、ROS 节点、范围、可写性、是否在单页测试界面显示以及所属正式 YAML。算法实际默认值仍由唯一的 `clearance_engine.yaml` 提供，不在装订表中复制一份。当前净空参数为 X 检测上下限、圆柱半径、高度带、最少支持点、YZ 网格、最少占用格和最小跨度；离线正式回放直接把 MCAP 原始点云送入净空节点。
