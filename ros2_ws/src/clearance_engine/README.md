...
高度计算。输入坐标固定为`x=East`、`y=North`、`z=Up`。当前首版只输出沿`Up`
方向从雷达原点到顶部的竖直高度，不计算路面到雷达中心的安装高度，
...
→ 法向与+Up夹角不超过15°的PCL多平面RANSAC
→ 0.10米East-North网格八邻域连通区域
→ 区域自身内点重新拟合与残差检查
→ 至少9×9跨度且实际占用不少于81格
→ 各区域实际覆盖网格内的最低预测高度
...
平面方程为`aE+bN+cU+d=0`，每个占用网格中心的向上高度为：
...
原朝向静态原始点云已经删除，不能用于本次Up坐标变更验收。重新采集的数据必须
同时包含原始点云和同步高频里程计。实车行驶时单帧原始点云覆盖约94毫秒，当前
节点已改接逐点姿态和平移补偿后的ENU点云。节点已通过
...
未保存点云，只在线观察新链路。关闭不可用的里程计平移并启用IMU重力校正后，连续
5秒收到52帧结果，52帧全部有效；雷达到顶面`Up`距离均值2.8746米、标准差0.0144
米、范围2.8428～2.9048米。该结果证明当前放置下Topic、时间插值、重力对齐和顶部
算法可以连续运行，但没有人工真值，也没有倾斜45°数据，不能作为精度或45°能力
验收。旧朝向数据已删除，后续必须重新采集新朝向原始点云进行正式验证。
