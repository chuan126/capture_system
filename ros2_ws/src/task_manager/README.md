...
其中雷达初始化阶段只记录流程状态，不等待或校验真实雷达、RTK数据。记录器创建成功后立即进入采集中。雷达安装高度、高度下限阈值和高度上限阈值的有效范围均为 `[0, 20] m`，`0 m` 是合法值；高度下限阈值不得大于高度上限阈值。圆柱检测半径范围为 `[0.1, 5.0] m`，最低簇支持点数范围为 `[1, 10000]`。开始时六项作业参数与车道一起冻结，其中两项算法参数以参数快照schema version 4保存，并随记录器请求写入任务测量元数据。
