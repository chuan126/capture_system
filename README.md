...
- 独立任务管理页面已经删除。任务创建入口只保留在采集首页任务控制卡片中，每次保存一个任务，保存成功后才能继续创建下一项；创建时填写隧道编号、隧道名称、作业车道、高度下限阈值和高度上限阈值。作业车道由上/下行方向和左/右车道组成，每个任务保存独立计划值；开始前可在任务控制卡片覆盖当前任务的车道和两个阈值，开始后由设备端冻结实际执行值。旧批次不再参与当前前端流程。
