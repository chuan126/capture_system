...
- 高德地图 Key 由 RK3588 统一配置，`securityJsCode` 仅在 FastAPI 的 `/_AMapService` 代理中使用，换浏览器无需重复配置。
...
- 独立任务管理页面已经删除。任务创建入口只保留在采集首页任务控制卡片中，每次保存一个任务，保存成功后才能继续创建下一项；创建时只填写隧道编号和隧道名称，不再要求用户管理作业批次。
