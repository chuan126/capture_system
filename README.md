...
- 数据回放页面读取任务测量 SQLite 文件时采用 prefix 首段和按视图加载的曲线接口。初始长任务只读取开头固定样本段，已加载窗口按 `sample_index` 合并缓存在当前页面；回拖到分辨率足够的已加载区域不重复请求，放大时只对分辨率不足的区域补读。显示降采样保留局部最低值、最高值和无效断点，正式 50 Hz 数据不改变。报告页保留正式 TXT 明细和原有 PDF 汇总，并在相邻按钮新增单任务 DeepSeek 大模型辅助报告；每次点击都将该任务 `measurements.db` 转为一份 JSON 并创建一次新的模型请求，不做长任务分块。正常停止且包含有效样本的正式记录可以导出。
