...
激光雷达和 RTK，在设备端完成原始雷达本体系最低可信点簇检测、原始点云预览、状态监控和
局域网网页展示。

当前算法输出 `lidar_to_top_m`，表示原始雷达本体系中沿安装定义 `+X` 高度轴的最低
可信点簇中位距离。任务记录器叠加任务开始时冻结的雷达安装高度生成最终净空；本算法
不再使用 IMU、里程计、四元数或运动补偿点云。`+X` 轴语义仍须通过现场标定复核。
...
└→ cloud_visualization（原始 X-Y-Z，仅预览）
...
网页点云预览从原始雷达本体系点云分出独立显示旁路，不做旋转、平移或运动补偿。预览节点只在有网页消费者时处理
当前帧，过滤非有限点和明确零占位点；有效点不超过 10000 时全部显示，超过后使用体素优先的
空间限点。它按同一时间戳和原始线性索引把全扫描点、圆柱 ROI、有效最低可信点簇分别编码为
蓝、绿、红；标签缺失或错帧时安全退化为全蓝。该旁路不反馈到净空算法。当前生产配置不使用
厂商 SLAM 点云作为网页预览输入。

## 已实现能力

- ODIN1 Lite 厂商驱动接入及稳定 `/capture/...` Topic remapping。
- 原始雷达本体系圆柱 ROI、最低高度带和 YZ 网格连通支持判断，输出最低可信点簇中位高度。
- RTK 串口接入、NMEA 解析结果发布和网页字段映射。
- 原始 RTK 串口接入、状态、地图轨迹和正式端点记录；融合定位与航位推算已删除。
- 雷达、RTK、RK3588 资源和数据目录容量的统一系统诊断。
- 原始雷达 X-Y-Z 点云的 5 Hz、最多 10,000 点 PCV2 蓝/绿/红旁路网页预览。
...
- 隧道入口和出口RTK端点由普通用户在采集中或已暂停时手动记录。只有最近 2 s 内收到且有效的 Fix 才写入已确认端点；失败重录不覆盖此前成功坐标。开始不依赖RTK，停止固定测量边界后立即收尾，不等待RTK。
...
- 独立任务管理页面已经删除。任务创建入口只保留在采集首页任务控制卡片中，每次保存一个任务，保存成功后才能继续创建下一项；创建时填写隧道编号、隧道名称、作业车道、高度下限阈值和高度上限阈值。新任务的作业车道提供上/下行各右1至右4共八项，右1固定表示最右侧车道，编号向左递增；每个任务保存独立计划值，开始前可在任务控制卡片覆盖当前任务的车道和两个阈值，开始后由设备端冻结实际执行值。历史左/右车道字段只用于原样读取旧任务，不自动换算为右起编号。旧批次不再参与当前前端流程。
- 数据回放页面读取任务测量 SQLite 文件时采用 prefix 首段和按视图加载的曲线接口。初始长任务只读取开头固定样本段，已加载窗口按 `sample_index` 合并缓存在当前页面；回拖到分辨率足够的已加载区域不重复请求，放大时只对分辨率不足的区域补读。显示降采样保留局部最低值、最高值和无效断点，正式 50 Hz 回放序列不改变。报告页的TXT改为按IMU实际接收频率读取schema v13 `imu_samples`并输出38列；原有PDF汇总与单任务DeepSeek大模型辅助报告并列。DeepSeek API、Key、模型和Skill保存在设备runtime设置中，每次点击创建一次新的模型请求。RK3588只读流式扫描最大2 GiB的任务数据库，先在本地形成固定大小的统计与最低候选证据包，再以单次请求发送给模型，不上传数据库全表。
- 开始和停止均为单击执行，不显示确认弹窗。停止完成后按创建时间自动选中下一项待执行任务，但不会自动开始。
- 数据回放页面支持单项、多项、按日期和当前筛选结果全选后永久删除。普通用户确认一次后，FastAPI 删除对应任务目录并保留中央索引中的删除时间和释放空间；采集中和已暂停任务不能删除。独立物理清理接口仅保留为旧调用兼容入口。
...
开发阶段一键启动雷达、RTK、原始点云净空算法、系统监控、点云预览和网页服务：
