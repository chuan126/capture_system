...
本文说明当前前端已落地的页面结构和接口边界。历史高度记录、普通用户批量永久删除、正式文件生成和设备端任务控制均已接入 FastAPI。
...
任务创建时同时保存独立的计划作业车道、高度下限阈值和高度上限阈值。选中待执行任务后，采集首页任务控制卡片加载计划值；操作人员可在开始前覆盖这些参数，开始后实际执行值冻结并锁定。两个阈值定义含边界的正常区间。约 16 英寸笔记本视口下，雷达安装高度、作业车道、高度下限阈值和高度上限阈值使用两列两行排版。
...
数据回放和报告导出任务列表按创建时间倒序显示，日期分组和同日任务均将最后创建的任务放在最上方。

页面包含以下固定区域。

- 左侧任务搜索、状态筛选、日期分组和任务选择；
- 中部当前任务按真实时间窗口加载的50 Hz最近源帧保持曲线；
...
任务浏览器按 `display_id` 的日期部分分组。用户可以勾选单个任务、选择某一天的全部可删除任务，也可以对当前搜索和筛选结果执行全选。采集中和已暂停任务的复选框禁用。客户回放页面调用 `POST /api/v1/tasks/delete-selected`，一次确认后永久删除对应本地任务目录；后端在删除前检查全部所选任务，任何一个任务不满足条件时不会开始清理。

单任务 `DELETE /api/v1/tasks/{task_id}` 使用相同的永久删除语义。`POST /api/v1/tasks/purge-data` 仅保留给旧调用和历史逻辑删除数据清理。
...
“原始数据保存”TXT以每帧实际收到的IMU消息为一行，固定38列，包括记录时间、隧道与车道、实时/累计最低高度、入口/出口RTK、逐样本RTK质量、IMU六轴、雷达温度、最低点XYZ、兼容姿态列、ODIN位置和四元数。列间固定使用4个ASCII空格，缺失项输出`0`。旧数据库没有`imu_samples`时回退到历史保持序列。文件名使用任务时间编号，例如`20260807_145601_T-001_原始数据保存.txt`。
...
任务开始时保存任务控制卡片当时的实际行驶方向、右起车道编号、高度下限阈值和高度上限阈值。新任务的右1为最右侧车道，编号向左递增；历史左右车道保留原始语义，不自动换算。PDF由后端基于距离域局部基准、低值事件连续性、最低点空间轨迹和周期性结构保护计算原始最低值与有效最低值，只排除高置信异常；待复核值仍保守计入。入口和出口 RTK
