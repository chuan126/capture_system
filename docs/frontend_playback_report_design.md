...
本文说明当前前端已落地的页面结构和接口边界。历史高度记录、任务逻辑删除、本地数据
物理清理、正式文件生成和设备端任务控制均已接入 FastAPI。
...
任务浏览器按 `display_id` 的日期部分分组。用户可以勾选单个任务，也可以选择某一天的全部
可见任务。物理清理调用 `POST /api/v1/tasks/purge-data`，实际删除所选任务目录并保留中央任务
索引、UUID、时间编号和清理记录。活动任务不能清理，后端对整次请求返回冲突，不做静默部分
清理。

当前任务删除仍使用 `DELETE /api/v1/tasks/{task_id}`，属于逻辑删除，不保证释放磁盘空间。
