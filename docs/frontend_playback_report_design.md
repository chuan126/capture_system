...
50 Hz测量明细TXT每个最近源帧保持样本一行，固定48列，包括源帧追溯、RTK时间/纬经高/质量、隧道与车道、实时/最低高度、入口/出口RTK、20 ms区间IMU三轴平均值、雷达温度、最低点XYZ、车辆姿态、ODIN位置和四元数。列间固定使用4个ASCII空格。旧数据库没有新增字段时保持兼容空值。文件名使用任务时间编号，例如`20260807_145601_T-001_50Hz测量明细.txt`。
