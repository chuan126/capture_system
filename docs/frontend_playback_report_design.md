...
任务创建时同时保存独立的计划作业车道和高度阈值。作业车道在界面中以“上行左车道、上行右车道、下行左车道、下行右车道”四种组合显示，数据库内部拆分为行驶方向和左右车道。选中待执行任务后，采集首页任务控制卡片加载计划值；操作人员可在开始前覆盖车道和阈值，开始后实际执行值冻结并锁定。回放和正式报告使用实际执行值。
...
最低高度、入口/出口RTK、50 Hz IMU三轴平均值、雷达温度、最低点XYZ、ODIN姿态和里程计位置。无效高度和无有效
...
任务开始时保存任务控制卡片当时的实际行驶方向和左右车道；若操作人员在开始前覆盖了创建时的计划值，报告显示覆盖后的实际执行车道。最低高度由后端从测量数据库有效样本计算。入口和出口 RTK
