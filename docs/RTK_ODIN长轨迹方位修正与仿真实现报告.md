...
1. `simulation_test_mode` 在定位节点参数声明和YAML中实现：0仿真，1真实RTK，默认1。
2. A参数：`simulation_rtk_point_a=[24.5738888889,118.0894444444,20.0]`。
3. B参数：`simulation_rtk_point_b=[24.5741666667,118.0897222222,20.0]`。
4. 默认A/B分别为北纬24度34分26秒、东经118度05分22秒和北纬24度34分27秒、
   东经118度05分23秒，高度均20 m；水平距离约41.8 m。
...
19. 默认A/B只有41.8 m，低于50 m临时门限和100 m正式门限；验证
    `heading_fit_valid=true` 时必须把B设置为距A至少100 m，并实际移动相应距离。
20. 人工步骤：YAML设模式0和A/B；重启ODIN与正式定位；静止观察进度约0%；沿近似直线移动；
    观察进度、样本、基线、拟合角和修正后误差；到B确认100%；结束后恢复模式1并重启。
