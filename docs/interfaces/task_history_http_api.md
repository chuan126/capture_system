...
删除会物理移除 `CAPTURE_DATA_ROOT/tasks/<task_id>/`，同时填写 `deleted_at`、
`delete_reason`、`local_data_purged_at` 和 `purged_bytes`。普通任务查询默认排除已删除记录，
中央索引、UUID和时间编号保留用于审计，时间编号不重排、不回收。
...
后端先读取并检查全部任务及其目录路径，再开始物理删除；任一任务不存在、已经删除、正在采集、处于暂停状态或占用活动槽时，整批请求失败且不开始清理。成功后在同一个 SQLite 事务中保存删除审计字段，返回删除数量、释放字节数和任务 UUID 列表。单任务 `DELETE /api/v1/tasks/{task_id}` 执行相同的永久删除语义。

## 4. 兼容物理清理接口

该接口只为旧调用和历史逻辑删除任务保留；当前前端删除已经直接释放本地空间：
...
接口按 `task_id` 查找任务，不排除历史已删除记录。后端先检查全部任务；任何任务处于活动状态时整次请求返回 HTTP 409，不静默跳过。成功时删除仍存在的 `CAPTURE_DATA_ROOT/tasks/<task_id>/`，并保留中央任务索引、UUID、时间编号、状态历史和清理时间。
...
设备端记录时间戳，前端据此把首段样本显示为初始时间窗口。初始化视窗不会触发 `series`；只有用户主动改变
窗口后才请求局部数据。完整统计在首段曲线出现后异步读取，不阻塞首屏。

`summary` 返回测量统计、数据来源、实际检测方向与车道、入口出口 RTK、暂停区间数量以及首尾
样本索引和设备端记录时间戳，不返回 `samples`。

`series` 使用 `recorded_timestamp_ns` 对应的设备端记录时间窗口查询，保持暂停间隔和真实日期时间。
雷达 `source_timestamp_ns` 仅用于源帧关联和诊断，不能解释为Unix时间。`max_points` 允许 200 至 10000，默认
4000。窗口样本不超过上限时返回全部样本；超过上限时由 SQLite 按记录时间桶直接选择局部最低值、
...
完整记录时间范围。用户拖拽或缩放离开首段后，前端再调用 `series` 读取当前视图窗口；“回到开头”恢复
首段窗口，不主动触发整任务曲线扫描。
...
`recorded_timestamp_ns` 对应的毫秒时间窗口读取；新测量库为该字段建立索引，旧测量库无需迁移也可读取。
暂停造成的真实时间间隔仍保留在横轴中。
