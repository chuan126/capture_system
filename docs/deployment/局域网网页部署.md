...
> 当前边界：网页任务控制只修改浏览器内存状态，尚未连接设备端 `task_manager`。
> 数据回放和报告导出仍是占位与测试下载功能。
...
- `/api/v1/report-export-test`：模拟报告下载链路测试；
...
浏览器不直接连接 ROS 2。网络断开不会停止已经启动的ROS 2节点；正式设备端任务状态机尚未实现。
...
- 净空和系统状态协议、WebSocket以及采集首页任务控制源码结构；
- 前端 lint；
