...
采集首页已连接四路 WebSocket。任务创建和查询通过 FastAPI HTTP 接口写入设备端 SQLite，浏览器不直接访问 ROS 2。任务切换仅改变页面选择。开始、暂停和停止仍是浏览器内存原型。
数据回放已通过 FastAPI 读取每任务测量 SQLite 文件。设备端测量写入和正式文件生成接口尚未实现。
