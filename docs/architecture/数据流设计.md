...
`data_recorder` 保存两组净空数据。`clearance_source_frames` 保留算法实际源帧；measurement schema v4 起源帧诊断字段使用 `candidate_region_count`、`selected_grid_area_m2`、`selected_residual_median_m` 和 `selected_residual_p95_m`，分别表示合格连通区域数量、水平投影网格覆盖面积、中位残差和 P95 残差。当前新记录为 measurement schema v5，并在 `recording_metadata` 保存实际行驶方向和左右车道。`clearance_samples` 以 50 Hz 写入最近源帧保持结果。每条 50 Hz 记录保存源帧时间、序号、
