...
原始点云直接供正式净空算法。点云补偿和里程计只供网页预览旁路；预览用精确源时间戳和
原始线性索引合并诊断分类，丢失时退全蓝。当前链路不使用 SLAM 点云进行网页显示。
...
`data_recorder` 保存两组净空数据。`clearance_source_frames` 保留算法实际源帧；measurement schema v4 起的候选面积、平面残差和 ENU 最低点字段继续保留数据库兼容，但原始本体系点簇算法将不适用字段保存为空值，不冒充平面或 ENU 坐标。簇点数写入兼容内点计数字段，真实代表点本体系 XYZ 只在 `RawClearanceDiagnostics` 中实时提供，当前正式任务库不新增该旁路大索引消息。当前新记录为 measurement schema v12；`recording_metadata` 保存实际行驶方向、左右车道和冻结高度参数，`clearance_samples` 以50 Hz保存最近源帧保持序列，并显式记录来源、重复序号、逐样本RTK质量、IMU/定位旁路字段和最终安装高度修正值。只根据可靠源序号或设备时间戳拒绝通信层重复源帧，不根据数值相同去重。暂停期间不写正式样本。
...
- 原始点云时间、frame、布局或空间支持不足时净空输出无效；位姿不足只影响预览旁路；
