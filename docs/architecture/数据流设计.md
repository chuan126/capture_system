...
`_02`、`_03`。永久删除历史任务不会导致编号重排。设备端 `task_manager` 通过 Service 编排记录器，
...
浏览器断线后设备端任务继续。任务创建时中央数据库保存每个任务独立的计划行驶方向、右起车道编号、高度下限阈值和高度上限阈值；右1为最右侧车道并向左递增。选择待执行任务时前端加载计划值，开始前允许现场覆盖。点击开始后，`task_manager` 将卡片当前值冻结为实际执行参数，并传给 `data_recorder`。历史左右车道只保留兼容读取，不推断为编号车道。开始采集前由 FastAPI 检查统一系统诊断，
