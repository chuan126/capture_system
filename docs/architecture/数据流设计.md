...
flowchart LR
    ...
    VIS["cloud_visualization\n保留最新帧、5 Hz限频、字段裁减、限点"]
    BRIDGE["FastAPI ROS 2 bridge\nPCV1封装、客户端限流"]
    WS["统一 WebSocket"]
    UI["Three.js\n传感器局部坐标、X轴向上"]

    RAW --> VIS --> BRIDGE --> WS --> UI
```

当前预览保持原始点云的传感器局部XYZ数值，不订阅里程计，不做时间戳配对、
坐标转换、无效点过滤、空间ROI或体素降采样。`cloud_visualization`按字段偏移和
消息步长检查输入，再生成连续XYZ预览点云；浏览器以+X轴为竖直向上方向，沿
+X轴提供YZ平面的俯视图。
...
断线；浏览器不订阅 ROS 2，也不接收带RGB的原始SLAM点云。
