...
`data_recorder` 保存两组净空数据。`clearance_source_frames` 保留算法实际源帧；measurement schema v4 起源帧诊断字段使用 `candidate_region_count`、`selected_grid_area_m2`、`selected_residual_median_m` 和 `selected_residual_p95_m`，分别表示合格连通区域数量、水平投影网格覆盖面积、中位残差和 P95 残差。当前新记录为 measurement schema v7；`recording_metadata` 保存实际行驶方向和左右车道，`clearance_samples` 以 50 Hz 写入最近源帧保持结果，并保存RTK时间、约400 Hz IMU周期平均值、雷达温度、最低点XYZ、ODIN姿态与位置。每条记录还保存源帧时间、序号、源年龄、重复标志和重复序号。没有源帧时写入 `source_unavailable`，源帧超过 250 ms 时写入 `source_timeout`。暂停期间不写 50 Hz 样本。
