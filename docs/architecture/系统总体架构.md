...
开始采集前 FastAPI 使用统一系统诊断检查设备上线条件。雷达要求原始点云持续有新帧，RTK 要求串口诊断为正常；任一设备未上线时只禁止 `start`，暂停、继续、停止和恢复仍按各自 Service 可用性独立判断。入口与出口 RTK 坐标只确认最近 2 s 内实际收到的有效 Fix；没有 Fix、Fix 无效或 Fix 超时时保存未确认状态并继续已经开始的任务。记录器保存实际净空源帧和 50 Hz 最近源帧保持序列。重复样本保存来源字段。
