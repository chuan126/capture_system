...
class RtkBridge:
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            node.create_subscription(
                ...
                lambda message: self._on_diagnostics(message, DiagnosticStatus.OK),
                qos,
            )
    ...
    def _on_diagnostics(self, message: object, ok_level: int | bytes) -> None:
        raw_ok_level = ok_level
        normalized_ok_level = (
            raw_ok_level[0] if isinstance(raw_ok_level, bytes) else int(raw_ok_level)
        )
        for status in getattr(message, "status"):
            if getattr(status, "name") != "rtk_driver/serial":
                continue
            raw_level = getattr(status, "level")
            level = raw_level[0] if isinstance(raw_level, bytes) else int(raw_level)
            self._snapshot = with_serial_diagnostic(
                self._snapshot,
                connected=level == normalized_ok_level,
