...
开始时，前端提交当前车道、雷达安装高度和高度阈值，由设备端冻结参数。雷达安装高度和高度阈值允许为 `0 m`，每次前端系统启动默认均为 `0 m`。开始流程显示雷达初始化、入口 RTK 快照、记录文件创建和正式记录阶段，但不等待雷达或 RTK 真实数据确认。入口 RTK 无有效坐标时显示“坐标未确认”，任务仍继续。
...
不会沿用旧值。实时曲线继续直接显示 `lidar_to_top_m`。顶部摘要和高度阈值判定使用 `lidar_to_top_m + 雷达安装高度`；低于阈值时当前净空数值显示红色。正式记录不回写算法结果，而是在独立字段 `clearance_height_m` 中保存安装高度修正后的净空高度。
...
50 Hz 记录由设备端记录器使用最近净空源帧保持生成。TXT包含RTK时间、隧道与车道、实时/最低高度、入口/出口RTK、50 Hz IMU平均值、雷达温度、最低点XYZ、ODIN俯仰/横滚/方位和里程计位置XYZ。
