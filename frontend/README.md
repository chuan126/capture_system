...
1. 50 Hz 测量明细 TXT。面向当前任务，每个采样周期一行，固定输出24个业务数据列；
...
50 Hz 记录由设备端记录器使用最近净空源帧保持生成。TXT包含RTK时间、隧道与车道、实时/最低高度、入口/出口RTK、50 Hz IMU平均值、雷达温度、最低点XYZ、ODIN俯仰/横滚/方位和里程计位置XYZ；这些数值测量列没有实际值时导出字面值 `0`，数据库中的 `NULL` 不改变。PDF 的最低高度只从任务冻结正常区间内的算法有效样本取最小值，低于高度下限阈值和超过高度上限阈值的样本均剔除；无正常区间样本的任务不进入 PDF，但仍可导出 TXT。
