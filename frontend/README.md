...
3. 大模型辅助分析 PDF。用户在报告页直接输入 DeepSeek API Key，Key 与模型选择保存在当前电脑浏览器的 `localStorage`。点击现有“导出 PDF”旁边的“大模型报告导出”后，只分析当前任务；每次点击创建一个新的持久化导出作业和一次新的 DeepSeek 对话请求，不续用上一次模型上下文。

TXT 文件名使用任务时间编号，例如 `20260807_145601_T-001_50Hz测量明细.txt`。PDF 文件名使用报告生成时间。报告不再依赖作业批次状态。界面测试数据、异常中断、记录不完整或无有效高度的任务不会进入正式文件。大模型报告固定先列出任务序号、隧道编号、检测车道、隧道最低净空高、可信度、记录时间和入口/出口 RTK，再给出文字分析；表格中的最低净空和可信度由设备端确定性程序计算，DeepSeek 不改写正式结果。

第一版大模型报告由 RK3588 只读打开当前任务的 `measurements.db`，按表名、列名和行数据转换成紧凑 JSON，并通过一次非流式 `/chat/completions` 请求发送给 DeepSeek，不上传 SQLite 二进制文件、不做长任务分块，也不截断数据。单窗口 JSON 超出设备配置上限时作业明确失败。API Key 按本项目当前交付要求直接保存在显示端；异步作业运行期间还会暂存在设备端作业索引，完成、失败、取消或服务恢复时删除，不写入报告 manifest。
