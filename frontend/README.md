...
采集首页任务卡片只保留“创建任务”入口。创建窗口每次只录入一个任务，填写隧道编号、隧道名称、作业车道、高度阈值和高度上限。点击“保存并继续创建”时，当前任务必须先成功写入后端，页面才清空进入下一项。后端为每条任务生成稳定 UUID 和创建时间显示编号，例如 `20260807_145601`；同一秒连续创建多个任务时追加 `_02`、`_03`。删除历史任务不会改变或重置后续编号。

任务控制通过 FastAPI HTTP 接口连接设备端 `task_manager`。前端不直接创建 ROS 2客户端，也不自行把任务改为采集中、已暂停或已停止。

卡片结构：

1. 作业参数栏：雷达安装高度、作业车道、高度阈值和高度上限，采用双列两行排版；
2. 当前任务：时间编号、隧道编号、隧道名称、车道和文字状态；
3. 待测任务：按创建时间正序显示。连续创建任务时，当前任务保持为最早尚未执行的任务；设备端确认任务完成后再自动选择下一项；
4. 固定控制区：开始采集、暂停或继续、停止。

开始时，前端提交当前车道、雷达安装高度、高度阈值和高度上限，由设备端冻结参数。四项参数在约 16 英寸笔记本视口下保持两列布局和 40 px 高控件，卡片中部滚动，底部控制按钮固定可见。高度阈值和高度上限范围均为 `[0, 20] m`，并要求阈值不大于上限；高度上限默认 `20 m`。开始流程显示雷达初始化、入口 RTK 快照、记录文件创建和正式记录阶段，但不等待雷达或 RTK 真实数据确认。入口 RTK 无有效坐标时显示“坐标未确认”，任务仍继续。
...
不会沿用旧值。实时曲线继续直接显示 `lidar_to_top_m`。顶部摘要使用 `lidar_to_top_m + 雷达安装高度`，只有位于“高度阈值 ≤ 净空高度 ≤ 高度上限”的值属于正常；低于阈值或超过上限时当前净空数值均显示红色并标明原因。正式记录不回写算法结果，而是在独立字段 `clearance_height_m` 中保存安装高度修正后的净空高度。
...
50 Hz 记录由设备端记录器使用最近净空源帧保持生成。TXT包含RTK时间、隧道与车道、实时/最低高度、入口/出口RTK、50 Hz IMU平均值、雷达温度、最低点XYZ、ODIN俯仰/横滚/方位和里程计位置XYZ；这些数值测量列没有实际值时导出字面值 `0`，数据库中的 `NULL` 不改变。PDF 的最低高度只从任务冻结正常区间内的算法有效样本取最小值，低于阈值和超过上限的样本均剔除；无正常区间样本的任务不进入 PDF，但仍可导出 TXT。
