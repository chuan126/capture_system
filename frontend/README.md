...
- 当前坐标：仅在 RTK WebSocket 正常、fix 有效且 RMC 不为 `V` 时显示 WGS84
  经纬度；
- 异常高度：任务正式采集中，当实时显示净空高度低于高度阈值时显示最近一次异常值，并使用红色字体警示；同一任务的新异常覆盖上一异常。
...
不会沿用旧值。实时曲线继续直接显示 `lidar_to_top_m`。顶部摘要和异常高度判定使用 `lidar_to_top_m + 雷达安装高度`。正式记录不回写算法结果，而是在独立字段 `clearance_height_m` 中保存安装高度修正后的净空高度。
