...
type Props = {
  ...
  onToggleAllChecked?: (taskIds: string[]) => void;
  sortOrder?: "asc" | "desc";
};
...
export default function TaskBrowser({tasks,selectedTaskId,onSelectTask,heading="任务列表",selectable=false,showSelectAll=false,checkedTaskIds=new Set(),disabledTaskIds=new Set(),onToggleChecked,onToggleDateChecked,onToggleAllChecked,sortOrder="desc"}:Props){
  ...
  const visibleSelectableIds=useMemo(()=>visibleTasks.filter(task=>!disabledTaskIds.has(task.taskId)).map(task=>task.taskId),[disabledTaskIds,visibleTasks]);
  const allVisibleChecked=selectable&&visibleSelectableIds.length>0&&visibleSelectableIds.every(id=>checkedTaskIds.has(id));
  const groups=useMemo(()=>{const direction=sortOrder==="asc"?1:-1;const map=new Map<string,CollectionTask[]>();for(const task of visibleTasks){const key=taskDateKey(task);const list=map.get(key)??[];list.push(task);map.set(key,list);}return [...map.entries()].sort(([a],[b])=>direction*a.localeCompare(b)).map(([key,list])=>[key,[...list].sort((a,b)=>direction*a.createdAt.localeCompare(b.createdAt))] as const);},[sortOrder,visibleTasks]);
  return <aside className="panel task-browser" aria-label={heading}>
    <header className="task-browser__head"><div><span>任务记录</span><strong>{heading}</strong></div><div className="task-browser__head-actions"><small>{visibleTasks.length} 项</small>{selectable&&showSelectAll&&onToggleAllChecked&&<button type="button" disabled={visibleSelectableIds.length===0} onClick={()=>onToggleAllChecked(visibleSelectableIds)}>{allVisibleChecked?"取消全选":"全选"}</button>}</div></header>
    ...
    <div className="task-browser__list task-browser__list--dated">
      {groups.length===0?<div className="task-browser__empty"><strong>{tasks.length===0?"当前没有任务":"没有匹配任务"}</strong><span>{tasks.length===0?"请先在采集首页创建任务":"请调整搜索条件"}</span></div>:groups.map(([dateKey,dateTasks])=>{
        const ids=dateTasks.filter(task=>!disabledTaskIds.has(task.taskId)).map(t=>t.taskId); const allChecked=selectable&&ids.length>0&&ids.every(id=>checkedTaskIds.has(id));
        return <section className="task-browser-date" key={dateKey}>
          <header className="task-browser-date__head"><strong>{formatTaskDateKey(dateKey)}</strong><span>{dateTasks.length} 项</span>{selectable&&onToggleDateChecked&&ids.length>0&&<button type="button" onClick={()=>onToggleDateChecked(dateKey,ids)}>{allChecked?"取消当日":"选择当日"}</button>}</header>
          {dateTasks.map(task=>{const selectionDisabled=disabledTaskIds.has(task.taskId);return <div className={`task-browser-row${task.taskId===selectedTaskId?" active":""}`} key={task.taskId}>
            {selectable&&<input type="checkbox" aria-label={`选择 ${task.displayId}`} checked={checkedTaskIds.has(task.taskId)} disabled={selectionDisabled} title={selectionDisabled?"采集中或已暂停的任务不能删除":undefined} onChange={()=>onToggleChecked?.(task.taskId)}/>}
