...
import {
  ...
  generateTaskTxt,
  loadReportPreview,
} from "../components/report/reportExportApi.ts";
...
const filePayload = {
  export_format: "txt",
  file_name: "任务01_G45-001_50Hz测量明细.txt",
  file_size_bytes: 1234,
  generated_at: "2026-08-06T01:02:00Z",
...
test("loads report exportability from FastAPI without ROS 2 browser access", async () => {
  ...
  assert.equal(preview.tasks[0].minimumHeightM, 5.18);
});

test("creates TXT and PDF through FastAPI HTTP endpoints", async () => {
  const calls = [];
  globalThis.fetch = async (url, init) => {
    calls.push([url, init.method]);
    const payload = url.includes("clearance-summary")
      ? { ...filePayload, export_format: "pdf", file_name: "隧道净空检测汇总报告.pdf", report_id: "report-1", task_id: null, included_task_count: 1 }
      : filePayload;
  ...
  assert.deepEqual(calls, [
    ["/api/v1/tasks/task-1/exports/txt", "POST"],
    ["/api/v1/reports/clearance-summary", "POST"],
  ]);
});
